...
Thread::~Thread()
{
    ...
    DEBUG(dbgSys, "Deleting thread: " << name);
...
void Thread::SetChildExitCode (int childpid, int ecode)
{
   unsigned i;
...
Thread::Exit (bool terminateSim, int exitcode)
{
    ...
    while (nextThread == NULL) {
       ...
       else kernel->interrupt->Idle();      // no one to run, wait for an interrupt
       nextThread = kernel->scheduler->FindNextToRun();
    }
...
//     for (i = 0; i < 8; i++)
//     {
//         if (childArrayStatus[i]==TRUE)
//         {
//             kernel->currentThread = childArrayThread[i];
//         }
//     }
// }
...
static void LoadAndExecuteFile(char *filename)
{
    DEBUG(dbgSys, "FILENAME CHECK: " << filename);
    if (filename != NULL)
    {
...
{
    DEBUG(dbgSys, "Forking Thread " << this->getName());
