...
#ifdef RDATA
    DEBUG(dbgSys, "code = " << noffH->code.size << " readonly = " << noffH->readonlyData.size << " init = " << noffH->initData.size << " uninit = " << noffH->uninitData.size << "\n");
#endif
}
...
bool AddrSpace::Load(char *fileName)
{
    ...
    size = noffH.code.size + noffH.initData.size + noffH.uninitData.size + UserStackSize; // we need to increase the size
                                                                                          // to leave room for the stack
#endif
    DEBUG(dbgSys, "PageSize, SIZE: " << PageSize << ", " << size);
    ...
    {
        DEBUG(dbgSys, "Initializing code segment.");
    ...
    {
        DEBUG(dbgSys, "Initializing data segment.");
        DEBUG(dbgSys, noffH.initData.virtualAddr << ", " << noffH.initData.size);
        executable->ReadAt(
    ...
    {
        DEBUG(dbgSys, "Initializing read only data segment.");
        DEBUG(dbgSys, noffH.readonlyData.virtualAddr << ", " << noffH.readonlyData.size);
        executable->ReadAt(
            &(kernel->machine->mainMemory[noffH.readonlyData.virtualAddr]),
            noffH.readonlyData.size, noffH.readonlyData.inFileAddr);
    }
#endif
    DEBUG(dbgSys, "NUM FREE PAGE: " << NumFreePages);
    printPageTable();
...
void AddrSpace::InitRegisters()
{
    ...
    kernel->machine->WriteRegister(StackReg, numPages * PageSize - 16);
    DEBUG(dbgSys, "Initializing stack pointer: " << numPages * PageSize - 16);
}
...
AddrSpace::Translate(unsigned int vaddr, unsigned int *paddr, int isReadWrite)
{
    ...
    if (pfn >= NumPhysPages)
    {
        DEBUG(dbgSys, "Illegal physical page " << pfn);
